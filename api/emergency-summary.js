...
사용자는 지금 심정지가 의심되는 응급 상황에 처해 있고, 가장 가까운 AED 정보만 알고 있습니다.
...
export default async function handler(req, res) {
  ...
  const { deviceName, deviceAddress, distanceMeters } = req.body || {}
  ...
  const userContent = `가장 가까운 AED: ${deviceName}
주소: ${deviceAddress || '정보 없음'}
현재 위치에서 거리: 약 ${Math.round(distanceMeters)}m`
